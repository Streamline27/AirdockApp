...
class AppModule(private val ctx : Context) {
    ...
    @Singleton @Provides fun authorizationApi(generator: ApiGenerator) = generator.createService(AuthorizationApi::class.java, shouldAddToken = false)
    @Singleton @Provides fun taskApi(generator: ApiGenerator) = generator.createService(TaskApi::class.java)
}